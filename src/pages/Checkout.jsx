...
export default function Checkout() {
  const { cart, products, placeOrder, canFulfill, clearCart, customerId, walletBalance, company } = useStore();
  ...
  const confirmPay = async () => {
    ...
    await new Promise((r) => setTimeout(r, method === "wallet" ? 300 : 1100));
    const { order, error: err } = await placeOrder({ items, paymentMethod: methodLabel, source: "online" });
    setProcessing(false);
    if (err) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 sm:px-6 py-10">
      ...
      <button
        onClick={confirmPay}
        disabled={!method || processing || walletShort}
